...
RuleStatus ParallelFor::execute(ElementMap& game_state) {
    ...
    for(auto& [player_connection, player_list]: *player_maps) {
        ...
        game_state[element_name] = player_list;
...
When::When(Condition_Rules conditionExpression_rule_pairs)
...
std::string formatString(std::string str, std::string resolved_text) {
    if (!resolved_text.empty()) {
        size_t open_brace = 0; 
        while ((open_brace = str.find("{", open_brace)) != std::string::npos) {
            size_t close_brace = str.find("}", open_brace);
            str.replace(open_brace, close_brace-open_brace+1, resolved_text);
        }
    }
    return str;
}
...
InputChoice::InputChoice(std::string prompt, 
                        std::shared_ptr<ASTNode> text_expression_root,
    ...
    : prompt(prompt), text_expression_root(text_expression_root),
...
RuleStatus InputChoice::execute(ElementMap& game_state) {
    ...
    if (!awaiting_input[player_connection]) {
        ...
        text_expression_root->accept(resolver, game_state);
        std::stringstream formatted_prompt(formatString(prompt, resolver.getResult()->getString()));
...
GlobalMsg::GlobalMsg(std::string msg, std::shared_ptr<ASTNode> text_expression_root, 
                    std::shared_ptr<std::deque<std::string>> global_msgs)
    : msg(msg), text_expression_root(text_expression_root), global_msgs(global_msgs) {
}

RuleStatus GlobalMsg::execute(ElementMap& game_state) {
    LOG(INFO) << "* GlobalMsg Rule *";
    text_expression_root->accept(resolver, game_state);
    global_msgs->push_back(formatString(msg, resolver.getResult()->getString()));
    return RuleStatus::Done;
}
